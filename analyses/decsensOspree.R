...
plot(jitter(leafout_date)~expected_temp, pch=19, log="y", 
     ylab="Days to event",
     xlab=expression(paste("Temperature (", degree, "C)")),
     main="Sims: Non-linear (logged y)", 
     font.main = 1, cex.main = cex.mainhere, cex.lab=cex.labhere,
     bty="l", mgp=c(1.5, 0.25, 0), tck=-.01)
plot(jitter(leafout_date)~expected_temp, pch=19, log="xy", 
     ylab="Days to event",
     xlab=expression(paste("Temperature (", degree, "C)")),
...
if(FALSE){
    plot(response.time~forceday, data=char2, pch=19, log="y", 
     ylab="Days to budburst",
     xlab=expression(paste("Temperature (", degree, "C)")),
