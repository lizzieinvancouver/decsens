...
Many biological hypotheses explain the observed decline in plant sensitivity as a consequence of declines in over-winter chilling or short photoperiods \citep{fu2015,zohner2016,fu2019}. These forces generally change the thermal sum required for leafout each year \citep{Polgar2014,zohner2017,flynn2018}, weakening the linear relationship between biological responses and temperature. \\

When $\beta(t)$ (the within-year sensitivity) changes over time---increasing or decreasing---the regression coefficient from regressing leafout date on spring-time temperature ($\tilde{\beta}$, the observed, overall `sensitivity') is likely smaller. This follows from the total law of covariance:
\begin{align*}
\tilde{\beta} = Cov(Y, X) / Var(X) = [Cov(E(Y | t), E(X | t)) + E(Cov(Y, X | t))] / Var(X) < E(Cov(Y, X | t) / Var(X)) % = E(\beta(t))
\end{align*}

This assumes $X$ and $\beta$ are independent, $Var(X|t) = Var(X)$, and $Cov(E(Y|t), E(X|t)) > 0$. The first assumption would be false if mean temperatures ($X$) are correlated with over-winter chilling, or other covariates that lead to shifts in $\beta(t)$. % The latter is often true because increased mean temperatures ($X$) lead to earlier leafout dates ($Y$). Then, likely $Var(X|\beta) \leq Var(X)$ and then the inequality above could be reversed.
