...
When the thermal sum required for leafout ($\beta$ in our first hitting-time model above) increases the regression coefficient from regressing leafout date on spring-time temperature increases in magnitude, that is, it leads to increases (not declines) in sensitivity (Fig. \ref{fig:simshiftcuesfstar}). This happens because a higher thermal sum decreases the variance in temperature, and increases the covariance between temperature and leafout day. 
