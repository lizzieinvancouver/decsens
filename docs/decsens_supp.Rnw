...
\title{Supplemental materials: The illusion of declining temperature sensitivity with warming} 
...
In our model, leafout day, $n_\beta$, is the day, $n$, that cumulative daily temperature, $S_0^n$, hits the threshold, $\beta$. For simplicity, we assume researchers 
know the day temperatures start to accumulate, $n=0$. However, our model is easily extended to the case where researchers choose a window of temperatures, for example March 1 to April 30.
...
\subsection{The inverse relationship between leafout time and average temperature at leafout time}
...
Much research uses linear regression to quantify the relationship between $n_{\beta}$ and $M_0^{n_{\beta}}$. Regressing $n_{\beta}$ on $M_0^{n_{\beta}}$ finds a best fit line to the inverse curve, ${n_{\beta}} = \frac{\beta}{M_0^{n_{\beta}}}$. The relationship can be linearized with a logarithm transformation: $\log ({n_{\beta}}) = \log(\beta) - \log (M_0^{n_{\beta}})$. That is, $\log(n_{\beta})$ is linear in log-average daily temperature with slope -1 and intercept $\log(\beta)$ (e.g., in simulations in Fig. \ref{fig:basicsimswpep} is -1, and intercept is $\log(200)=5.3$).

\subsection{Declining sensitivity is explained by the declining variance of the average temperature}
