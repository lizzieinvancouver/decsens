...
\section{Model of leafout timing}
Following a general understanding of how warm temperatures (forcing) triggers leafout in temperate deciduous trees \citep{chuineJTB}, we define leafout day, $n_\beta$, as the day, $n$, that cumulative daily temperature, $S_0^n$, hits the threshold, $\beta$. Here we assume that we know the day temperatures start to accumulate, $n=0$ (below we extend to the case there is fixed window of temperatures).
...
We model $X_n$ as a Gaussian random walk, $X_n \overset{\text{i.i.d}}{\sim} \mathcal{N}(n \Delta, \sigma)$, where $\Delta > 0$ is the day-over-day increase in average temperatures and $\sigma$ is the standard deviation. Note that this model differs from the typical Gaussian random walk because of the factor $n$.\\

This model yields two important points: \\
...
Much research uses linear regression to quantify the relationship between $n_{\beta}$ and $M_0^{n_{\beta}}$. Regressing $n_{\beta}$ on $M_0^{n_{\beta}}$ finds a best fit line to the inverse curve, ${n_{\beta}} = \frac{\beta}{M_0^{n_{\beta}}}$. The relationship can be approximately linearized with a logarithm transformation: $\log ({n_{\beta}}) = \log(\beta) - \log (M_0^{n_{\beta}})$. That is, $\log(n_{\beta})$ is linear in log-average daily temperature with slope -1 and intercept $\log(\beta)$ (e.g., in simulations in Fig. \ref{fig:basicsimswpep} is -1, and intercept is $\log(200)=5.3$). In this example, where we stop accumulating $X$ based on $n_{\beta}$, an inverse would be more appropriate, and would be especially important if results are extrapolated (see Fig. \ref{fig:compareyxxy}). \\

(2) Declining sensitivity is explained by the declining variance of the average temperature (which, given the relationship between leafout time and average temperature also leads to declining variance in leafout date).\\

Under the model, the mean and variance of $M_0^n$ is E$(M_0^n) = \frac{1}{n} \sum_{i=0}^n i \Delta = \frac{(n + 1)}{2} \Delta $ and Var$(M_0^n) = \frac{\sigma^2}{n}$.\\

Consider two years, $A$ and $B$, where $A_0^n$ denotes the average temperature in year $A$ by day $n$ with increase $\Delta_A$ and $B_0^n$ the average temperature in year $B$ with increase $\Delta_B$. Suppose year $B$ is warmer than year $A$, $\Delta_A < \Delta_B$, so that $A_0^n$ typically gets to $\beta$ in $n_A$ steps and $B_0^n$ gets to $\beta$ in $n_B$ steps, $n_A  > n_B$. \\

Then Var$(A_0^{n_A}) =  \frac{\sigma^2}{n_A} < \frac{\sigma^2}{n_B}$ = Var$(B_0^{n_B})$.


\subsection{Adjusted random walk model for a temporal window of measured temperature}
...
Here we consider a scenario where there is a chosen a window of temperatures, for example March 1 to April 30. Let:
\begin{align*}
...
We model $X_n$ as a Gaussian random walk, $X_n \overset{\text{i.i.d}}{\sim} \mathcal{N} \left ( \alpha_0 + \alpha_1 n , \sigma \right )$. We assume $X_n > 0$ for all $n$ and $a < n_{\beta} < b$. \\ % Note: This is an important assumption ... I started to add text about this, but decided to skip it. We could, and probably best to reference \citep{gusewell2017} if do.
...
The mean and variance can be found from the identities \\
...
\caption{\textbf{Simulated leafout as a function of temperature across different temperatures highlights non-linearity of process.} Here we simulated sets of data where leafout constantly occurs at 200 growing degree days across mean temperatures of 0, 5, 10 and 20C (constant SD of 4), we calculated estimated mean temperature across a fixed window (top row, similar to estimates of 'spring temperature') or until leafout date (bottom row). While within any small temperature range the relationship may appear linear, its non-linear relationship becomes clear across the greater range shown here (left). Taking the log of leafout (middle) reduces this some, but taking the log of both leafout and temperature (right) further linearizes the relationship.}
\label{fig:simslog} % decsensSimsAuerbach.R
\end{figure}

\begin{figure}[h!]
\centering
\noindent \includegraphics[width=0.9\textwidth]{..//analyses/figures/compareyxxy.pdf}
\caption{\textbf{Extrapolation of regression from simulated data given models of leafout day versus average daily temperature} where leafout day is regressed against log(average daily temperature), shown on left, gives poorer predictions than when modeling the inverse (log(leafout day) against mean average daily temperature), shown on right.}
\label{fig:compareyxxy} % decsensSimsAuerbach.R
\end{figure}
